...
const run = async () => {
            ...
            await forEach(files, async (file) => {
                ...
                if (stat.isFile()) {
                    const s3Path = path.join(
                        outDir,
                        path.relative(config.source, fullPath),
                    );
    ...
    const baseDomain = config.cdnDomain
        ? config.cdnDomain
        : config.endpoint
          ? config.endpoint.replace("{region}", config.spaceRegion)
          : `${config.spaceName}.${config.spaceRegion}.digitaloceanspaces.com`;

    const outputPath = `https://${baseDomain}/${outDir}`;
